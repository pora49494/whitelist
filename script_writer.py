    ...
    for protocol in wl:
        for port in wl[protocol]:
            for ip in wl[protocol][port]["secured_ip"]:
                fw_hard.write("sudo iptables -A FORWARD -p {} --dport {} -d {} -s {} -j ACCEPT\n".format(protocol, port, device_ip, ip))
                fw_hard.write("sudo iptables -A FORWARD -p {} --dport {} -s {} -d {} -j ACCEPT\n".format(protocol, port, device_ip, ip))
                fw_soft.write("sudo iptables -A FORWARD -p {} -d {} -s {} -j ACCEPT\n".format(protocol, device_ip, ip))
                fw_soft.write("sudo iptables -A FORWARD -p {} -s {} -d {} -j ACCEPT\n".format(protocol, device_ip, ip))

# DROP
for device_ip in devices_whitelist:
    fw_hard.write("sudo iptables -A FORWARD -p tcp -d {} NFQUEUE --queue-num 1\n".format(device_ip))
    fw_hard.write("sudo iptables -A FORWARD -p tcp -s {} NFQUEUE --queue-num 1\n".format(device_ip))
    fw_hard.write("sudo iptables -A FORWARD -p udp -d {} NFQUEUE --queue-num 1\n".format(device_ip))
    fw_hard.write("sudo iptables -A FORWARD -p udp -s {} NFQUEUE --queue-num 1\n".format(device_ip))
    
    fw_soft.write("sudo iptables -A FORWARD -p tcp -d {} NFQUEUE --queue-num 1\n".format(device_ip))
    fw_soft.write("sudo iptables -A FORWARD -p tcp -s {} NFQUEUE --queue-num 1\n".format(device_ip))
