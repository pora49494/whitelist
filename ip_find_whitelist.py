    ...
    try: 
        ...
        if eth.type == dpkt.ethernet.ETH_TYPE_IP \
          and ( ip.p == dpkt.ip.IP_PROTO_UDP or ip.p == dpkt.ip.IP_PROTO_TCP ) : 
            ...
            device_side = check_device_side(src_ip, dst_ip)
            # PPDS : Protocol | Port | Device's IP | Server's IP
            if device_side == 'src' : 
                ppds = "{}-{}-{}-{}".format(protocol, port, src_ip, dst_ip)
                if ppds not in servers_status : 
                    servers_status[ppds] = True 

            elif device_side == 'dst' : 
                ppds = "{}-{}-{}-{}".format(protocol, port, dst_ip, src_ip)
                if ppds not in servers_status : 
...
for ppds in servers_record : 
    ...
    if secure_ratio > 0.90 : 
        protocol, port, device_ip, server_ip = ppds.split("-")   
        wl = devices_profile[device_ip]["white_list"]
...
with open('result/{}/ip_whitelist_{}.json'.format(filename,opcode), 'w') as fp:
