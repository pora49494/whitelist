...
fw = open('iptables_script_hard.sh', 'w')
    ...
    for protocol in wl:
        for port in wl[protocol]:
            for ip in wl[protocol][port]["secured_ip"]:
                fw.write("sudo iptables -A FORWARD -p {} --dport {} -m mac --mac-destination {} -s {} -j NFQUEUE --queue-num 1\n".format(protocol, port, device_mac, ip))
                fw.write("sudo iptables -A FORWARD -p {} --dport {} -m mac --mac-source {} -d {} -j NFQUEUE --queue-num 2\n".format(protocol, port, device_mac, ip))
